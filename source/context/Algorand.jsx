...
const Context = React.createContext({test:2});
...
function Provider(props) {
  ...
  const requestSignature = (transaction) => {
  ...
  const createFungibleToken = useCallback(
    async () => {
      ...
      const metadataDigest = await crypto.subtle.digest('SHA-256', textEncoder.encode(metadataString));
  ...
  return (
    <Context.Provider value={{
      ...
      {hasSignatureRequests &&
        <Signature onCancel={onCancelSignature} />
      }
